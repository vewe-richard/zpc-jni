...
#ifndef ZWAVE_COMMAND_CLASS_SWITCH_BINARY_HANDLERS_H
#define ZWAVE_COMMAND_CLASS_SWITCH_BINARY_HANDLERS_H
...
/**
 * @defgroup COMMAND_CLASS_SWITCH_BINARY_handler Command handlers for COMMAND_CLASS_SWITCH_BINARY
 * @brief Functions for controlling COMMAND_CLASS_SWITCH_BINARY
 * 
 * This module implement functions for generating and parsing the Z-Wave frames
 * for controlling COMMAND_CLASS_SWITCH_BINARY. The module builds frames by reading data from the
...
digraph G {
  ...
  COMMAND_CLASS_SWITCH_BINARY_VERSION [ label="COMMAND_CLASS_SWITCH_BINARY_VERSION\nV0\nget:", shape="ellipse" ]
  ep [ label="endpoint", shape="box"]
  SWITCH_BINARY_CURRENT_VALUE [ label="SWITCH_BINARY_CURRENT_VALUE\nV1\nget:SWITCH_BINARY_GET", shape="ellipse" ]
  ep [ label="endpoint", shape="box"]
  SWITCH_BINARY_TARGET_VALUE [ label="SWITCH_BINARY_TARGET_VALUE\nV2\nget:SWITCH_BINARY_GET", shape="ellipse" ]
  ep [ label="endpoint", shape="box"]
  SWITCH_BINARY_DURATION [ label="SWITCH_BINARY_DURATION\nV2\nget:SWITCH_BINARY_GET", shape="ellipse" ]

  ep -> COMMAND_CLASS_SWITCH_BINARY_VERSION
  ep -> SWITCH_BINARY_CURRENT_VALUE
  ep -> SWITCH_BINARY_TARGET_VALUE
  ep -> SWITCH_BINARY_DURATION
}
@enddot
...
/**
 * @brief Version of COMMAND_CLASS_SWITCH_BINARY which this handlers is genrated for
 * 
 */
#define COMMAND_CLASS_SWITCH_BINARY_VERSION 2


/**
 * @brief Initialisation of COMMAND_CLASS_SWITCH_BINARY
 * 
...
/**
 * @brief Override function for SWITCH_BINARY_REPORT
 * 
 ...
 */
sl_status_t  zwave_SWITCH_BINARY_REPORT_handle_report_command_override(
  const zwave_controller_connection_info_t *connection_info,
  const uint8_t *frame,
  uint16_t frame_length) ;

/**
 * @brief Override function for SWITCH_BINARY_GET
 * 
...
sl_status_t zwave_SWITCH_BINARY_GET_override(
  attribute_store_node_t  _node, uint8_t *frame, uint16_t *frame_len);

/**
 * @brief Override function for SWITCH_BINARY_SET
 * 
 * This if this function is implemented as a weak symbol. This provided such an
 * external component can modify the behaviour of the auto generated code. 
...
sl_status_t zwave_SWITCH_BINARY_SET_override(
  attribute_store_node_t _node, uint8_t *frame, uint16_t *frame_len);
...
#endif  //ZWAVE_HANDLERS_COMMAND_CLASS_SWITCH_BINARY_H
